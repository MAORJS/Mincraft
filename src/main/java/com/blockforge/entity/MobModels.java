...
public final class MobModels {
    ...
    public static void registerAll() {
        ...
        int cowHead = face(0x5E4230, 0x1A1A1A, 0xD8CDB5, s++);
        int cowLeg = hide(0x4A3524, -1, 0, s++);
        MODELS.put(MobType.COW, quadruped(12, 10, 18, 8, 8, 6, 10, cowBody, cowHead, cowLeg));

        // PIG
        int pigBody = hide(0xE8A0A8, -1, 0, s++);
        int pigHead = face(0xE8A0A8, 0x1A1A1A, 0xD87880, s++);
        int pigLeg = hide(0xD08890, -1, 0, s++);
        MODELS.put(MobType.PIG, quadruped(10, 8, 16, 6, 8, 8, 8, pigBody, pigHead, pigLeg));

        // SHEEP: woolly
        int sheepBody = hide(0xE8E6E0, -1, 0, s++);
        int sheepHead = face(0xD8CDB5, 0x1A1A1A, -1, s++);
        int sheepLeg = hide(0xC8BDA5, -1, 0, s++);
        MODELS.put(MobType.SHEEP, quadruped(11, 10, 16, 7, 7, 6, 10, sheepBody, sheepHead, sheepLeg));

        // CHICKEN: small white bird
        int chBody = hide(0xEFEDE5, -1, 0, s++);
        int chHead = face(0xEFEDE5, 0x1A1A1A, 0xE8A83E, s++);
        int chLeg = hide(0xE8A83E, -1, 0, s++);
        MODELS.put(MobType.CHICKEN, new Part[]{
                new Part(0, 5 / 16f, 0, -3 / 16f, 0, -4 / 16f, 6, 6, 8, Anim.NONE, chBody),
                new Part(0, 11 / 16f, -3 / 16f, -2 / 16f, 0, -2 / 16f, 4, 4, 4, Anim.HEAD, chHead),
        ...
        int zHead = face(0x4A8A5A, 0x1A1A1A, -1, s++);
        int zLimb = hide(0x35604A, -1, 0, s++);
        MODELS.put(MobType.ZOMBIE, humanoid(zBody, zHead, zLimb));

        // SKELETON: pale gray humanoid, thin limbs
        int kBody = hide(0xC8C8C0, 0xA8A8A0, 0.25f, s++);
        int kHead = face(0xD8D8D0, 0x101010, -1, s++);
        int kLimb = hide(0xC0C0B8, -1, 0, s++);
        Part[] sk = humanoid(kBody, kHead, kLimb);
        // slim the limbs
        ...
        int spHead = face(0x2E2826, 0xC42D2D, -1, s++);
        ...
        MODELS.put(MobType.SPIDER, new Part[]{
                ...
                new Part(0, 6 / 16f, -5 / 16f, -3 / 16f, 0, -4 / 16f, 6, 6, 6, Anim.HEAD, spHead),
    ...
    private static Part[] quadruped(float bw, float bh, float bl, float head,
                                    ...
                                    int bodyTile, int headTile, int legTile) {
        float legY = legH / 16f;
        ...
        return new Part[]{
                ...
                new Part(0, bodyY + bh / 40f, -bl / 32f - head / 40f,
                        -head / 32f, -head / 40f, -head / 16f, head, head, head, Anim.HEAD, headTile),
    ...
    private static Part[] humanoid(int bodyTile, int headTile, int limbTile) {
        return new Part[]{
                new Part(0, 18 / 16f, 0, -4 / 16f, -6 / 16f, -2 / 16f, 8, 12, 4, Anim.NONE, bodyTile),
                new Part(0, 24 / 16f, 0, -4 / 16f, 0, -4 / 16f, 8, 8, 8, Anim.HEAD, headTile),
